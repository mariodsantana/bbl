...
#define D(flags,...) if (flags & XT_BBL_VERBOSE) \
			fprintf(stderr, "bigblocklist: " __VA_ARGS__)
...
static void bbl_mt_help(void) {
	printf("Big IP Block List options:\n"
		"    --src --dst --both\n"
		"           match against packets' src and/or dst addresses\n"
		"    --0verbose\n"
		"           show my work.\n");
} /* bbl_mt_help */

static void bbl_mt_init(struct xt_entry_match *match) {
	struct bbl_mtinfo *info = (void *)match->data;
	info->bitfield = NULL;
	info->flags = 0;
	info->refcount = 0;
	info->dev = NULL;
...
static void bbl_mt_check(unsigned int flags) {
	...
	D(flags,"bigblocklist: checking flags: %u\n",flags);
	if (! (flags & (XT_BBL_SRC | XT_BBL_DST | XT_BBL_BOTH)) )
		xtables_error(PARAMETER_PROBLEM,
			"xt_bigblocklist: must specify src, dst, or both\n");
	if (flags & XT_BBL_VERBOSE) {
		fprintf(stderr,"bigblocklist: good flags: %x\n----[ ", flags);
		if (flags & XT_BBL_DST)
...
static void bbl_mt_save(const void *ip, const struct xt_entry_match *match) {
	const struct bbl_mtinfo *info = (const void *)match->data;
...
static struct xtables_match bbl_mt_reg = {
	...
	.size           = XT_ALIGN(sizeof(struct bbl_mtinfo)),
	.userspacesize  = XT_ALIGN(sizeof(struct bbl_mtinfo)),
	.init           = bbl_mt_init,
	.help           = bbl_mt_help,
	.parse          = bbl_mt_parse,
